...
best_mse_mae = 10000
for i in range(200):
    model.fit(x_train, [y_train, y_train_class], epochs = 1, batch_size = 1, validation_split = 0.2)
    ...
    if score[3] < best_mae:
        ...
        open('model_mt1.json', 'w').write(json_string)
        model.save_weights('weights_mt1.h5')
    if score[4] < best_mse:
        best_mse = score[4]
